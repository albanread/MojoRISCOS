...
from gamepane.audio.sfx import SFX_COUNT
...
comptime D_RING_WRITE = D_VOICE_BASE + 3 * D_VOICE_STRIDE
comptime D_RING_READ = D_RING_WRITE + 1
comptime D_RING_BASE = D_RING_READ + 1
"""The two ring cursors have SLOTS OF THEIR OWN, and that is the fix for a
real defect rather than a tidy-up.

They used to be addressed as `D_RING_BASE - 2` and `D_RING_BASE - 1`, i.e.
"the two slots just before the ring" -- but the ring began immediately after
the three voices, so those two slots were the last two of VOICE 2. Slot 18
was the ring's write cursor AND voice 2's `D_V_LEFT` countdown. While voice 2
held an effect, `advance_effects` decremented that countdown every 50Hz tick,
which walked the write cursor BACKWARDS; `drain_triggers` compares read
against write and simply stopped firing anything. The symptom is the worst
kind: sound effects work until one plays on the third voice, and then they
quietly stop."""
...
comptime MAX_BUFFER = 16384
"""Scratch for chip B, in SAMPLES.

It was 4096, described as "sized past anything WASAPI asks for", and it was
not: the stream is opened with `buffer_ms=90`, and the pre-roll asks the fill
for the WHOLE ring in one call -- 4320 frames at 48kHz. The very first fill
overran the allocation by 224 samples, every time, before a note had played.
16384 covers a 340ms buffer at 48kHz and costs 64KB; `gamepane_fill` also
clamps, so an endpoint that asks for more than this is quiet rather than
memory corruption."""
...
def sfx_play(d: P, effect: Int):
    ...
    var write = dget(d, D_RING_WRITE)          # ours; no other writer
    var read = dget_acquire(d, D_RING_READ)   # the callback's; acquire
    ...
    dput_release(d, D_RING_WRITE, write + 1)


def pending_triggers(d: P) -> Int:
    return dget(d, D_RING_WRITE) - dget(d, D_RING_READ)
...
def drain_triggers(d: P):
    ...
    var read = dget(d, D_RING_READ)           # ours; no other writer
    let write = dget_acquire(d, D_RING_WRITE)   # the game's; acquire
    let b = sfx_chip(d)
    while read < write:
        _start_effect(d, b, dget(d, D_RING_BASE + (read & (RING_SIZE - 1))))
        read += 1
    dput_release(d, D_RING_READ, read)
...
def gamepane_fill(
    ...
    # The scratch is a fixed allocation and this is the only place that can
    # overrun it. A stream that asks for more than it holds gets silence for
    # the excess rather than a write past the end -- the belt to MAX_BUFFER's
    # braces, because the pre-roll asks for the whole ring at once and the
    # ring's size comes from the endpoint, not from us.
    var n = frames
    if n > MAX_BUFFER:
        for i in range(MAX_BUFFER, n):
    ...
    if dget(d, D_TUNE) != 0:
        ...
        render_scheduled(a, dest, n)
    else:
        chip_render(a, dest, n, _player_tick_a)
    chip_render(b, scratch, n, _sfx_tick_ptr)
...
def start_audio(d: P) raises -> Int:
    ...
    g_deck()[] = Int(d)
    # SIX parameters, and the sixth is not optional. CreateThread is
    # (lpAttributes, dwStackSize, lpStartAddress, lpParameter, dwCreationFlags,
    # lpThreadId) -- this was declared with five, so kernel32 read whatever
    # happened to be at [rsp+0x28] as lpThreadId and, whenever that was not
    # NULL, wrote four bytes of thread id through it. A wild write on every
    # start_audio, landing somewhere else entirely.
    var create_thread = win32[
        def (
            Int, Int, Int, Int, UInt32, Int,
        ) thin abi("C") -> Int,
        "CreateThread",
    ]()
    var thread_proc: ThreadProc = gamepane_audio_thread
    var handle = create_thread(
        0,  # lpThreadAttributes: default security
        0,  # dwStackSize: default
        Int(_fn_ptr_as_opaque(thread_proc)),
        Int(d),  # lpParameter: the deck, as the thread's argument
        UInt32(0),  # dwCreationFlags: run immediately
        0,  # lpThreadId: not wanted, and now actually passed
    )
    if handle == 0:
