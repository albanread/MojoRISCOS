...
def play_tune_gm(source: String) raises -> Bool:
    ...
    # THE PROGRAM CHANGE, WHICH THIS PLAYER USED TO OMIT ENTIRELY.
    #
    # The whole reason a cue goes to the system synthesiser rather than to the
    # chip is that `%%MIDI program 52` should BE a choir. The parser reads that
    # line into `voices[v].instrument` and the SMF writer emits it -- but this
    # player walked only the note events and sent only 0x80/0x90, so every one
    # of Galaxigans' four cues played on program 0, a piano, whatever its
    # header said. Square lead, choir, glockenspiel and polysynth pad were all
    # the same instrument, and nothing anywhere reported it.
    #
    # A 0xC0 goes in at tick 0 for each voice that names an instrument, and it
    # sorts before every note because the tie rule below puts a zero-velocity
    ...
    for i in range(len(tune.events)):
        ...
        if ev.kind != EV_NOTE:
            continue
        if ev.velocity == 0:
            # `resolve_ties` silences the note that is tied INTO by setting
            # its velocity to zero, meaning "this is not a new note". Emitting
            # its note-off anyway cut the joined note exactly where the tie
            # had joined it. None of Galaxigans' cues uses a tie, so it never
            # bit here -- but `C-C` would have been chopped in half.
            continue
    ...
    while done < count and nheaders < MAX_HEADERS:
        ...
        if rc != UInt32(MMSYSERR_NOERROR):
            ...
            # nheaders was already PREPARED on the line above, so the
            # count has to include it -- otherwise stop_tune_gm unprepares
            # 0..nheaders-1 and then frees a block with one MIDIHDR still
            # prepared inside it.
            g_gm_header_count()[] = nheaders + 1
            stop_tune_gm()
            return False
        done += chunk
        nheaders += 1
